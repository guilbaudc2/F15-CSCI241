...
<body>
<?php
function youngestContact($people)
{asort($people);
			foreach($people as $peopleNames => $peopleInfo)
			{
...
?>
			<ul>
<?php				
			foreach($contacts[$_GET["id"]] as $information => $details)
			{
?>
				<li><?php echo ucfirst($information) ?>: <?php echo $details ?></li>
<?php
...
?>			
			<ul>
<?php	
			foreach($contacts as $contactNames => $contactInfo)
			{
	         if($contactInfo["state"] == $state)
             {
				echo "<li><a href='contacts.php?id={$contactNames}'>{$contactInfo["name"]} - {$contactInfo["city"]}, {$contactInfo["state"]}</a></li>";
		     } 
		     if ($contactInfo["age"])
			 {
			 	$ages[] =  $contactInfo["age"];
			 }
?>
			</ul>	
<?php		 
...
<?php		 
			...
			if($age < $averageAge)
			{
				echo "This person is younger than the average age of people on the contact list.";
			} else if ($age > $averageAge)
			{
				echo "This person is older than the average age of people on the contact list.";
			} else {
				echo "This person is the average age of people on the contact list.";
			}	
?>
			<br>
...
<?php
			...
			{
?>				
			<p>There are no contact details for this person.</p>
			<br>
			<a href='contacts.php'>Back to list</a>		
<?php			}
		...
		asort($contacts);
		$youngest = youngestContact($contacts);
		$oldest = oldestContact($contacts);
?>		
		<ul>
<?php			
		foreach($contacts as $contactNames => $contactInfo)
		{
			...
			if($contactInfo["age"] == $youngest)
			{
			echo "<li><a href='contacts.php?id={$contactNames}'>{$contactInfo["name"]} - {$contactInfo["city"]}, {$contactInfo["state"]}   -   YOUNGEST</a></li>";
		}	else if ($contactInfo["age"] == $oldest)
					{
			echo "<li><a href='contacts.php?id={$contactNames}'>{$contactInfo["name"]} - {$contactInfo["city"]}, {$contactInfo["state"]}   -   OLDEST</a></li>";
		} else {
			echo "<li><a href='contacts.php?id={$contactNames}'>{$contactInfo["name"]} - {$contactInfo["city"]}, {$contactInfo["state"]}</a></li>";
		}
		}
